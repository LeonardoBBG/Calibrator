...
def build_theme_store(
    ...
    Arguments are grouped by (effect, case_effect, confidence) and never removed.
    """
    ...
    return {
        ...
        "flat_matches": _build_flat_matches(theme_store),
    }
...
def _group_key(match: Dict) -> str:
    return f"{match['effect']}|{match['case_effect']}|{match['confidence']}"


def _add_match(theme: Dict, match: Dict) -> None:
    key = _group_key(match)
    theme["groups"].setdefault(key, {
        "effect": match["effect"],
        "case_effect": match["case_effect"],
        "confidence": match["confidence"],
        "n_matches": 0,
        "matches": [],
    })
    group = theme["groups"][key]
...
def _sort_store_matches(theme_store: Dict) -> None:
    for theme in theme_store.values():
        for group in theme["groups"].values():
            group["matches"].sort(
...
def _build_flat_matches(theme_store: Dict) -> List[Dict]:
    rows = []
    for theme_id, theme in theme_store.items():
        for group in theme["groups"].values():
            for match in group["matches"]:
                rows.append(_flatten_match(theme_id, theme["theme_label"], match))
    return rows
...
def _build_top_matches(theme_store: Dict, top_n_per_theme: int) -> List[Dict]:
    ...
    for theme_id, theme in theme_store.items():
        all_matches = [
            _flatten_match(theme_id, theme["theme_label"], match)
            for group in theme["groups"].values()
            for match in group["matches"]
        ]
        rows.extend(sorted(all_matches, key=lambda row: float(row["rank_score"]), reverse=True)[:top_n_per_theme])
    return rows
...
def _build_theme_summary(theme_store: Dict) -> List[Dict]:
    ...
    for theme_id, theme in theme_store.items():
        ...
        top_matches = sorted(all_matches, key=lambda item: item["rank_score"], reverse=True)
        group_keys = list(theme["groups"].keys())
        rows.append({
            "theme": theme_id,
...
def _review_priority_score(match: Dict) -> float:
    ...
    return round(
        abs(match["rank_score"])
        + (0.15 if match["case_effect"] == "ADVERSE" else 0)
        + (0.10 * confidence),
        4,
    )
...
def _flatten_match(theme_id: str, theme_label: str, match: Dict) -> Dict:
    row = dict(match)
    row["theme"] = theme_id
